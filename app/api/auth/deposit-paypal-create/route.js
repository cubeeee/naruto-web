...
// Tạo đơn nạp ticket qua PayPal → trả paypal_order_id để PayPal SDK mở popup thanh toán.
export async function POST(req) {
  ...
  if (!account_id || !role_id) {
    return Response.json({ ok: false, msg: "Please select a character first." }, { status: 400 });
  }
  if (!/^[0-9a-f]{24}$/i.test(role_id)) {
    return Response.json({ ok: false, msg: "Invalid character id." }, { status: 400 });
  }
  if (!Number.isInteger(tickets) || tickets < 1) {
    return Response.json({ ok: false, msg: "Please choose a valid amount." }, { status: 400 });
  }
  ...
  let res;
  try {
    res = await fetch(`${GAME_API_BASE}/ajax/create_deposit_paypal.php`, {
      method: "POST",
      headers: {
